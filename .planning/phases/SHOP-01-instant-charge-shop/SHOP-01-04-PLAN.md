...
<task type="auto">
  ...
  <action>
                ...
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:"var(--ink-60)",marginBottom:4}}>
                      ...
                      {charge.amountPending ? " 금액 미정" : ` ${(charge.amount||0).toLocaleString()}원`} ·
                  ...
                  <div style={{marginTop:8,padding:"8px 10px",background:"var(--blue-lt,rgba(59,130,246,0.08))",borderRadius:8,fontSize:12}}>
                    ...
                    <button className="btn btn-sm btn-secondary" style={{width:"100%"}}
                      onClick={async () => {
                        ...
                        const msg = `[RYE-K K-Culture Center]\n${studentName} 회원님, 추가 청구 안내드립니다.\n\n· ${charge.itemCategory} — ${charge.itemName}: ${(charge.amount||0).toLocaleString()}원\n\n· 카카오뱅크 3333-34-5220544\n  (예금주: 예케이케이컬처센터)\n입금 부탁드립니다. 감사합니다.`;
    ...
    **5. 승인 모달 추가** — `requestsModal &&` 블록 바로 다음, 즉시청구 요청 모달 앞에:
      ...
      <div className="mb" onClick={e => e.target === e.currentTarget && !approveInstantSaving && setApproveInstantModal(null)}>
        <div className="modal" style={{maxWidth:480}}>
            ...
            <button className="btn btn-primary" disabled={approveInstantSaving}
              onClick={async () => {
                ...
                if (!finalAmount || finalAmount <= 0) {
                  setApproveInstantMsg("");
                  // 인라인 에러 — window.alert 금지
                  const errEl = document.getElementById("approve-instant-err");
                  if (errEl) { errEl.textContent = "금액을 입력하세요 (0원 불가)"; setTimeout(() => { if(errEl) errEl.textContent = ""; }, 2500); }
                  return;
                }
                ...
                try {
                  ...
                  // 알림 메시지 자동 생성
                  const student = students.find(s => s.id === approveInstantModal.studentId);
                  const msg = `[RYE-K K-Culture Center]\n${student?.name || "회원"} 회원님, 추가 청구 안내드립니다.\n\n· ${approveInstantModal.itemCategory} — ${approveInstantModal.itemName}: ${finalAmount.toLocaleString()}원\n\n· 카카오뱅크 3333-34-5220544\n  (예금주: 예케이케이컬처센터)\n입금 부탁드립니다. 감사합니다.`;
          ...
          <div style={{padding:"0 20px 12px"}}>
            <span id="approve-instant-err" style={{fontSize:12,color:"var(--red)",fontWeight:500}}></span>
          </div>
        </div>
      </div>
    )}
    ...
    주의:
    - `window.confirm/alert` 금지. 에러는 `id="approve-instant-err"` DOM 조작 또는 별도 state로 처리.
...
<task type="auto">
  ...
  <action>
    1. **firebase.js import 수정** (line 2): `updateInstantCharge`를 추가한다:
       ```js
  ...
  <acceptance_criteria>
    - updateInstantCharge import: `grep -c "updateInstantCharge" src/App.jsx` → 최소 2 (import + 콜백)
